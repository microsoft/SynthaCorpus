...
// This module contains functions dealing with Unicode and UTF-8.
...
byte ascii_non_tokens[256] = {0}, ascii_alnums[256] = {0}, ascii_punct[256] = {0};
...
void initialise_ascii_tables(byte *non_token_string, BOOL include_cp1252_punctuation) {
  int a;
  ...
  // isalnum() replacement
  for (a = '0'; a <= '9'; a++) ascii_alnums[a] = 1;
  for (a = 'A'; a <= 'Z'; a++) ascii_alnums[a] = 1;
  for (a = 'a'; a <= 'z'; a++) ascii_alnums[a] = 1;

  // ispunct() replacement
  for (a = '!'; a <= '/'; a++) ascii_punct[a] = 1;
  for (a = ':'; a <= '@'; a++) ascii_punct[a] = 1;
  for (a = '['; a <= '`'; a++) ascii_punct[a] = 1;
  for (a = '{'; a <= '~'; a++) ascii_punct[a] = 1; 
}
...
void initialise_unicode_conversion_arrays(BOOL verbose) {
...
#include "toLower.txt"
  ...
  for (c = 0; c < CODE_POINTS_IN_BMP; c++) {
    if (utf8_bytes_needed(map_unicode_to_lower[c]) > utf8_bytes_needed(c)) {
      if (verbose) printf("initialise_unicode_conversion_arrays: Removing problematic length-increasing case transformation for %u\n", c);
      map_unicode_to_lower[c] = c;
      length_increases++;
...
unicode_t utf8_getchar(byte *str, byte **bafter, BOOL cp1252_conversion) {
  ...
  //
  // WARNING: cp1252_conversion is length increasing if any CP-1252 characters are encountered.
  //
  byte unary, *s = str;
  unicode_t rslt = 0;
...
int utf8_lower_case(u_char *string) {
  ...
  int len = 0;
  unicode_t ucs;
  u_char *r = string, *w = string, *next, *x;
  while (*r) {
    if (*r & 0x80) { // This weak test allows us to handle CP-1252 
      // Since string is UTF-8 this r marks the start of a UTF-8 string
      ucs = utf8_getchar(r, &next, FALSE);
      if (ucs > BMP_MASK) { // Make sure not to access beyond the mapping table.
	x = utf8_putchar(ucs, w);  // Leave these other non-BMP chars alone.
      } else {
	if (0) printf("  UCS %X -> %X, R offset = %zd , W offset = %zd -- ",
		      ucs, map_unicode_to_lower[ucs], r - string, w - string);
	x = utf8_putchar(map_unicode_to_lower[ucs], w);
      }
      r = next;
      w = x;
      if (w > r) {
	printf("Error in utf8_lower_case(): writing in advance of reading\n");
	exit(1);
      }
    } else {
  ...
  while (*r && room_left > 0) {
	...
	*w = 0;  // Don't copy any of it.
	return (w - dest);   // -------------------------->
      ...
      ucs = utf8_getchar(r, &next, TRUE);  // Potentially length-increasing
      if (ucs > BMP_MASK) { // Make sure not to access beyond the mapping table.
	w = utf8_putchar(ucs, w);  // Leave these other non-BMP chars alone.
      } else {
	w = utf8_putchar(map_unicode_to_lower[ucs], w);
...
int utf8_split_line_into_null_terminated_words(byte *input, size_t inlen, byte **word_starts,
					       int max_words, int max_word_bytes,
					       BOOL case_fold,  // case-fold line before splitting
					       BOOL remove_accents, // before splitting
					       BOOL maxwellize,  // Perform some heuristic substitutions 
					       BOOL words_must_have_an_ASCII_alnum
					       ) {
  // It is assumed that input is a string of length inlen which we are
  // allowed to write in. (To put NULs at the end of each word.)  *** It is also
  // assumed that the storage associated with inlen is actually inlen + 1 bytes,
  // so we can terminate the last word with a NUL if we need to.  Note
  // that input may contain NULs and ASCII control characters.  We
  // scan input and make an array of pointers to the word starts,
  // inserting a NUL at each word end.  (Note that a maximum is
  // imposed on the number of bytes in a word.  When this limit is
  // imposed, care is taken not to split up a UTF-8 sequence.)
  // Scanning stops either at the terminating NUL or at the first
  // ASCII control character.  Relies on ascii_non_tokens[] array
  // having been initialised.
  //
  // Make the i-th element of word_starts point to the first byte of
  // the i-th word and return the number (up to max_words) of words
  // found.

  byte *p = input, *q, *bafter, *wdstart, *last;
  ...
  while (p <= last) { // We'll break out if we reach any of a few stopping conditions
    ...
    while (p <= last) {
      ...
      // an indexable items or to the end of input.
      if (*p & 0x80) {	
    ...
    // p may point to an indexable ASCII char, or the first byte of indexable UTF-8, or
    // we may have come to the end of input
    if (p >= last) {
      *(last + 1) = 0;
      return wds;   // End of input :- we've finished.
    }
    ...
    while (p <= last) {  // Traverse the sequence of indexable characters.  On exit
                        // p will point either to the first byte of a non-indexable, or to end of input
      if (*p & 0x80) {
	...
	if (ascii_non_tokens[*p]) {
	  ...
	  // indexable ASCII
	  if (ascii_alnums[*p]) contains_ASCII_alnum = TRUE;
	  p++;
	}
      }
    }
    ...
    *p = 0;  // Null terminate the word -- We've declared we're going to do this
    ...
    if (len > max_word_bytes) {
      ...
      *q-- = 0;
      ...
      // q either points to the start of the word, or to an ASCII byte
      // or to the first byte of a UTF-8 character
      if (*q & 0x80) *q = 0;
    }
...
int utf8_count_words_in_string(u_char *input, BOOL case_fold,  // case-fold line before splitting
  ...
  //  *** Need to change to take a length rather than rely on termination by control char
...
void test_utf8_functions() {
  ...
  string[3] = 0xB8;
  string[4] = 0;
  unicode = utf8_getchar(string, &next, TRUE);
  ...
  unicode = utf8_getchar(string + 2, &next, TRUE);
  if (unicode != 0x438) {
  ...
  unicode = utf8_getchar(next, &next, TRUE);
  if (unicode != 'a') {
